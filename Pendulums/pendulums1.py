...
def residuals(func,yy,xx,v):
    """
    yy, xx are data
    v are the parameters of the function func
    """
    res= yy-func(xx,v)
...
def x2_p_value(res,s_y,dof):
    ...
    p_value1 = 1 - chi2.cdf(chi_square, dof)
    p_value2=chi2.cdf(chi_square, dof)
...
## Classes : most easy way to ULTRA SPAGHETTI CODE

class data_set():
    def __init__(self,path,label1,label2,func,d_func):
        self.t1,self.s1,self.t2,self.s2=np.loadtxt(path,unpack=True,delimiter=",")

        self.s_t1=self.s_t2=np.full_like(self.t1,0.001) # Must be adjusted
        self.s_s1=self.s_s2=np.full_like(self.s1,0.001) # Must be adjusted
    ...
    def fit(self):
        ...
        if (np.size(list(self.label2))>0): #For the second item
            ...
            # for i in range(5):
            #
            #     self.s_s2=sy_eff(self.d_func,self.params2,self.t2,self.s_t2,self.s_s2)
            #     self.popt, self.pcov = curve_fit(self.func, self.t2, self.s2,sigma=self.s_s2,absolute_sigma=True)
            #     self.params= self.popt
            #     self.s_params= np.sqrt(self.pcov.diagonal())
    ...
    def plot(self):
            ...
            if (np.size(list(self.label1))>0):
                plt.errorbar(self.t1,self.s1,self.s_t1,self.s_s1,fmt=".",label=self.label1)
            ...
            if (np.size(list(self.label2))>0):
                plt.errorbar(self.t2,self.s2,self.s_t2,self.s_s2,fmt=".",label=self.label2)
...
ds1=data_set(f_path1,"si","pure",para,line)
